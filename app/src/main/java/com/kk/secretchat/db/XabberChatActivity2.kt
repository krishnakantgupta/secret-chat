...
//cyberlinkk@xabber.org    kkcyberlinks
//appscoop@xabber.org   appscoop

class XabberChatActivity2 : AppCompatActivity(), ChatView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        replyView = findViewById<LinearLayout>(R.id.reply_view)
...
//        senderID = findViewById<EditText>(R.id.txtSender).text.toString()
        val username = "appscoop@xabber.org"
        val password = "appscoop"

        findViewById<View>(R.id.btnReverse).setOnClickListener { v ->
            txtUsername.setText(senderID)
            txtPassword.setText("kkcyberlink")
...
//        binding = DataBindingUtil.setContentView(this, R.layout.activity_product_details)
    ...
    private fun setTime() {
        if (txtMinut.text != null && txtMinut.text.toString().trim().length > 0) {
            ...
            if (value > 10 || value <= 0) {
                Toast.makeText(this@XabberChatActivity2, "Enter Less than 15 minute", Toast.LENGTH_SHORT).show()
            } else {
    ...
    private fun sendMessage() {
        ...
        if (message != null && message.toString().trim().isNotEmpty()) {
            ...
            presenter.sendMessage(message,replyChatId,"")
        }
    }
    ...
    override fun chatState(chatStates: ChatState) {
        runOnUiThread {
            when (chatStates) {
                ChatState.composing -> toolbar.title = "typing..."//toolbarMessage.setText("typing...")
    ...
    override fun onContextItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            R.id.copy -> {
                Toast.makeText(this, "0 clicked " + position, Toast.LENGTH_SHORT).show()
                doCopy()
            }
            R.id.reply -> {
                Toast.makeText(this, "1 clicked " + position, Toast.LENGTH_SHORT).show()
                doReply()
            }
            else -> {
